...
class my_model(nn.Module):
    ...
    def recon_2(self, x_wide, x_wide_norm, x_mid_norm):
        # attention
        residual = x_wide
        x_hat, _ = self.attention_layer(query=x_wide_norm, key=x_mid_norm, value=x_mid_norm,
                                        need_weights=False)  # attenion
        x_hat = x_hat + residual  # 残差
    ...
    def forward(self, batch):
        ...
        x1_narrow, x2_narrow, x3_narrow = self.linear_map(x1_narrow), self.linear_map(x2_narrow), self.linear_map(
            x3_narrow)
        ...
        x1_narrow, x2_narrow, x3_narrow = [i + self.positional_encoding.pe[:, :] for i in
                                           [x1_narrow, x2_narrow, x3_narrow]]
        print("位置编码：done")
